...
const ComparisonSection = ({ currentUser }) => {
  ...
  const ComparisonRow = ({ label, currentValue, compareValue, format = (v) => v, icon = '' }) => {
    ...
    return (
      <div className="grid grid-cols-3 gap-4 py-3 border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition">
        <span className="font-medium flex items-center gap-2">
          {icon && <span>{icon}</span>}
          {label}
        </span>
        <span className={`text-right font-semibold ${isHigher ? 'text-green-500' : ''}`}>
          {current}
          {isHigher && <span className="ml-1 text-xs">↑</span>}
        </span>
        <span className={`text-right font-semibold ${isLower ? 'text-red-500' : ''}`}>
          {compare}
          {isLower && <span className="ml-1 text-xs">↓</span>}
        </span>
      </div>
    );
  };
    ...
    <div className="glass-card p-6">
      ...
      <div className="flex gap-2 mb-6">
        <input
          ...
          className="flex-1 px-4 py-3 rounded-lg glass-card focus:outline-none focus:ring-2 focus:ring-purple-500"
          onKeyPress={(e) => e.key === 'Enter' && handleCompare()}
        />
        <button
          onClick={handleCompare}
          disabled={loading}
          className="px-6 py-3 gradient-bg text-white rounded-lg hover:shadow-lg transition disabled:opacity-50 font-medium"
        >
          {loading ? (
      ...
      <AnimatePresence>
        {compareUser && (
          ...
          >
            <div className="grid grid-cols-3 gap-4 py-3 font-semibold text-gray-600 dark:text-gray-400 border-b-2 border-gray-300 dark:border-gray-600">
              <span>Metric</span>
              <span className="text-right flex items-center justify-end gap-1">
                <img src={currentUser.avatar_url} alt={currentUser.login} className="w-5 h-5 rounded-full" />
                {currentUser.login}
              </span>
              <span className="text-right flex items-center justify-end gap-1">
                <img src={compareUser.avatar_url} alt={compareUser.login} className="w-5 h-5 rounded-full" />
                {compareUser.login}
              </span>
            </div>
            ...
            {compareStats?.topStarredRepo && (
              <div className="mt-4 p-3 bg-purple-50 dark:bg-purple-900/20 rounded-lg">
                <p className="text-xs text-gray-500 mb-1">🏆 Top Starred Repository</p>
                <p className="text-sm font-semibold">{compareStats.topStarredRepo.name}</p>
                <p className="text-xs text-gray-500">⭐ {compareStats.topStarredRepo.stargazers_count} stars</p>
              </div>
            )}
          </motion.div>
        )}
      ...
      {!compareUser && !loading && compareUsername && (
        <p className="text-center text-gray-500 py-4">Enter a username above to start comparing</p>
      )}
    </div>
  );
};
